...
def s3_setup():
    ...
    n_time = time.time()
    time_delta = 10000000
    output_contract = contract.Contract(branch='master', parent='bluth', child='cornballer',state='raw')
    ...
    client.upload_file(Bucket=ENV_BUCKET, Filename= t_file_old.name, Key= output_contract.get_key(), ExtraArgs={"Metadata":{"source_modified_time": str(n_time - time_delta)}})
    ...
    client.upload_file(Bucket=ENV_BUCKET, Filename= t_file_new.name, Key= output_contract.get_key(), ExtraArgs={"Metadata":{"source_modified_time": str(n_time + time_delta)}})
...
def test_push_to_s3_updated_file(mock_validate):
    ...
    s3_time = float(client.head_object(Bucket=ENV_BUCKET, Key=params[2].get_key())['Metadata']['source_modified_time'])
...
def test_push_to_s3_not_if_older(mock_validate):
    ...
    s3_time_before = float(client.head_object(Bucket=ENV_BUCKET, Key=params[2].get_key())['Metadata']['source_modified_time'])
    extract.push_to_s3(tmp_dir=os.path.dirname(params[1].name),
                       output_contract=params[2])
    params[2].set_file_name(os.path.split(params[1].name)[1])
    s3_time_after = float(client.head_object(Bucket=ENV_BUCKET, Key=params[2].get_key())['Metadata']['source_modified_time'])
