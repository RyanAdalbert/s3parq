...
BEGIN;
	...
	VALUES
		...
		(SELECT id FROM pipeline_states WHERE pipeline_state_type_id = (SELECT id FROM pipeline_state_types WHERE name = 'metrics') AND pipeline_id = (SELECT id FROM pipelines WHERE name = 'sun_allbrands_patient_status')),
		0,
		'njb@integrichain.com'),
		((SELECT id FROM transformation_templates WHERE name = 'patient_status_metrics_time_to_first_fill'),
