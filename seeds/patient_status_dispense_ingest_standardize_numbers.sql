...
BEGIN;
    ...
    VALUES
        ((SELECT id
            ...
            WHERE name = 'dispense_standardize_numbers'), (SELECT id
            FROM pipeline_states
            WHERE pipeline_state_type_id = (SELECT id
                FROM pipeline_state_types
                WHERE name = 'ingest') AND pipeline_id = (SELECT id
                FROM pipelines
                WHERE name = 'alkermes_vivitrol_patient_status')), 2, 'jtobias@integrichain.com');
        ...
        ('input_transform', (SELECT id
            ...
            WHERE (pipeline_state_id IN (SELECT id
                ...
                WHERE (id NOT IN (SELECT t.id
                    ...
                    WHERE tt.name = 'dispense_standardize_numbers')))
            ORDER BY id LIMIT 1), 'dispense_ingest_column_mapping', 'jtobias@integrichain.com'),
