BEGIN;
	...
	VALUES
		('patient_status_ingest_standardize_numbers',
		'{"input_transform":{"datatype": "string", "description": "Name of transform to input data from"},
...
BEGIN;
	...
	VALUES
		((SELECT id FROM transformation_templates WHERE name = 'patient_status_ingest_standardize_numbers'),
		(SELECT id FROM pipeline_states WHERE pipeline_state_type_id = (SELECT id FROM pipeline_state_types WHERE name = 'ingest') AND pipeline_id = (SELECT id FROM pipelines WHERE name = 'sun_ilumya_patient_status')),
		2,
		'njb@integrichain.com'),
		((SELECT id FROM transformation_templates WHERE name = 'patient_status_ingest_standardize_numbers'),
		(SELECT id FROM pipeline_states WHERE pipeline_state_type_id = (SELECT id FROM pipeline_state_types WHERE name = 'ingest') AND pipeline_id = (SELECT id FROM pipelines WHERE name = 'sun_odomzo_patient_status')),
		2,
		'njb@integrichain.com'),
		((SELECT id FROM transformation_templates WHERE name = 'patient_status_ingest_standardize_numbers'),
