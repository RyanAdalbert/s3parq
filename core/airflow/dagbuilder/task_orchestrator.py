...
class TaskOrchestrator(LoggerMixin):
    ...
    def do_orchestrate(self)->None:
        ...
        for state in self._pipeline.pipeline_states:  
            self.logger.debug(f"Ordering transforms in state {state.pipeline_state_type.name}...")
            transformations = self._order_transformations_within_group(state.transformations)
            self.logger.debug(f"Done ordering transforms for state {state.pipeline_state_type.name}. {len(transformations)} sets of transforms created.")    
            all_transforms = []
        ...
        self.logger.debug(f"Applying dependancies to tasks for dag {self._dag.dag_id}...")
        self._tasks = self._apply_deps_to_ordered_tasks(all_pipeline_tasks, self._dag)    
        self.logger.info(f"Done orchestrating tasks. {len(self._tasks)} orchestrated.")
...
## PRIVATE ##
    ...
    def _order_transformations_within_group(self, transformations:list)->list:
        ...
        for t in transformations:
            ...
            self.logger.debug(f"added task with graph order # {t.graph_order} to group {graph_cursor}")
