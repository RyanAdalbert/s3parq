...
class DagBuilder(LoggerMixin):
    ...
    def _get_pipelines(self, only_active:bool = True) -> list:
        ...
        """
        self.logger.debug(f"getting {'active' if only_active else 'all'} pipelines from config...")
        ...
        self.logger.debug(f"Done getting pipelines, {len([x for x in pipelines])} pipelines found.")
        return pipelines
